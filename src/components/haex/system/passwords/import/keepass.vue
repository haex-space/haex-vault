<template>
  <HaexSystemPasswordsImportWizardShell
    ...
    :file-label="t('kdbxFile')"
    accept=".kdbx"
    :extra-valid="!!password"
...
// Reset the password whenever the dialog is closed — the shell resets its
// own file/error state on close, but the password ref lives here.
watch(open, (v) => {
  if (!v) password.value = ''
})
...
/**
 * KeePass-specific OTP extraction: pulls from three possible sources in order
 *   1. `otp` / `OTP` field (otpauth URI or bare secret)
 *   2. `TOTP Seed` + `TOTP Settings` field pair (KeePassXC layout, settings
 *      `period;digits;algorithm` — see KeePassXC docs)
 *   3. an `otpauth://` URI embedded in the notes (some exporters do this)
 *
 * The shared `parseOtpData` only covers the simple URI / bare-secret case.
 */
...
type ImportStats = { groupCount: number; entryCount: number }

const successDescription = (stats: ImportStats) =>
  t('successDescription', { groups: stats.groupCount, entries: stats.entryCount })

async function doImport(file: File, setProgress: (pct: number) => void): Promise<ImportStats> {
  if (!password.value) throw new Error(t('error.noPassword'))
  try {
    const buffer = await file.arrayBuffer()
    return await importKdbxAsync(buffer, password.value, setProgress)
  }
  catch (err) {
    const msg = err instanceof Error ? err.message : String(err)
    // kdbxweb surfaces wrong-password as InvalidKey / "Invalid password";
    // translate to a user-friendly message so the inline error makes sense.
    if (msg.includes('InvalidKey') || msg.includes('password')) {
      throw new Error(t('error.wrongPassword'), { cause: err })
    }
    throw err
  }
}
