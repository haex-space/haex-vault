...
export interface ColumnChange {
   ...
   * table stores `haex_column_sigs_no_sync` AND the scanner had a `spaceId`
...
/** Legal shape of the per-row `haex_column_sigs_no_sync` JSON payload:
...
/**
 * Gets all CRDT-enabled tables (tables with the `haex_hlc_no_sync` tracking column).
 */
...
async function getTableColumnsAsync(tableName: string) {
  ...
  // `haex_column_sigs_no_sync` is only present on the 6 tables migration 0012
...
async function processRowsToChangesAsync(
  ...
  for (const row of rows) {
    ...
    // vault, extension tables without haex_column_sigs_no_sync). The JSON shape
    ...
    if (opts.hasColumnSigs) {
      ...
      if (typeof sigsRaw === 'string' && sigsRaw.length > 0) {
        try {
          ...
          log.warn(
            `Failed to parse haex_column_sigs_no_sync for ${tableName} row ${JSON.stringify(
    ...
    for (const col of dataColumns) {
      ...
      if (!hlcToUse) {
        // This should never happen as every row must have haex_hlc_no_sync
        log.warn(`Column ${col.name} has no HLC and row has no haex_hlc_no_sync, skipping`)
        continue
      }
...
export async function scanTableForSpaceChangesAsync(
  ...
  const query = `SELECT ${columnList} FROM "${tableName}" t `
    ...
    + `ON i."id" = m."identity_id" AND i."private_key" IS NOT NULL `
    + `WHERE json_extract(a."haex_column_sigs_no_sync", ?) = i."did" `
    + `AND json_extract(a."haex_column_sigs_no_sync", ?) = i."did" `
    + `AND json_extract(a."haex_column_sigs_no_sync", ?) = i."did" ${hlcFilter}`
