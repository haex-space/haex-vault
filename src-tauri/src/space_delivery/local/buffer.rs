...
pub fn fetch_messages(
    ...
    let rows = core::select(
        "SELECT id, sender_did, message_type, message_blob, created_at_no_sync, \
...
pub fn consume_key_package(
    ...
    let rows = core::select(
        "SELECT id, package_blob, pop_blob FROM haex_local_delivery_key_packages_no_sync \
         ...
         ORDER BY created_at_no_sync ASC LIMIT 1"
            .to_string(),
        vec![
            serde_json::Value::String(space_id.to_string()),
            serde_json::Value::String(target_did.to_string()),
...
/// Deletes oldest packages first (by created_at_no_sync).
    ...
    core::execute(
        "DELETE FROM haex_local_delivery_key_packages_no_sync \
         WHERE id IN ( \
             ...
             WHERE space_id = ?1 AND target_did = ?2 \
             ORDER BY created_at_no_sync DESC \
             LIMIT -1 OFFSET ?3 \
         )"
        .to_string(),
        vec![
...
pub fn fetch_welcomes(
    ...
    let rows = core::select(
        "SELECT id, welcome_blob FROM haex_local_delivery_welcomes_no_sync \
         ...
         ORDER BY created_at_no_sync ASC"
            .to_string(),
        vec![
            serde_json::Value::String(space_id.to_string()),
            serde_json::Value::String(recipient_did.to_string()),
