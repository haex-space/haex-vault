...
///   sides" guidance).
/// - `haex_passwords` — a CRDT table (`ensure_crdt_columns` adds `haex_hlc_no_sync` /
///   `haex_column_hlcs_no_sync`). `discover_crdt_tables` picks it up via `haex_hlc_no_sync`,
...
pub(super) fn seed_vault_db(
    ...
    // Make haex_passwords a CRDT table (adds haex_hlc_no_sync + haex_column_hlcs_no_sync) so it
...
/// Insert a CRDT-tracked `haex_passwords` row, writing `haex_hlc_no_sync` /
/// `haex_column_hlcs_no_sync` directly (the columns the scanner reads) at a fixed HLC
...
pub(super) fn delete_password(db: &DbConnection, row_id: &str, delete_log_id: &str, hlc: &str) {
    ...
    conn.execute(
        "INSERT INTO haex_deleted_rows (id, table_name, row_pks, haex_hlc_no_sync, haex_column_hlcs_no_sync) \
         VALUES (?1, ?2, ?3, ?4, ?5)",
...
pub(super) fn update_password_secret(db: &DbConnection, row_id: &str, new_secret: &str, hlc: &str) {
    ...
    conn.execute(
        "UPDATE haex_passwords SET secret = ?1, haex_hlc_no_sync = ?2, haex_column_hlcs_no_sync = ?3 \
         WHERE id = ?4",
