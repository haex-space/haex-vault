...
//! `haex_column_hlcs_no_sync`: `haex_column_hlcs_no_sync` is the per-column HLC of the last
...
fn seed_row(db: &DbConnection, id: &str, name: &str, column_hlcs: &str, row_hlc: &str) {
    ...
    conn.execute(
        "INSERT INTO devices (id, name, haex_hlc_no_sync, haex_column_hlcs_no_sync) VALUES (?, ?, ?, ?)",
...
/// Parse the row's `haex_column_hlcs_no_sync` JSON map.
fn column_hlcs_map(db: &DbConnection, id: &str) -> serde_json::Map<String, JsonValue> {
    let guard = db.0.lock().unwrap();
    let conn = guard.as_ref().unwrap();
    let raw: String = conn
        .query_row(
            "SELECT haex_column_hlcs_no_sync FROM devices WHERE id = ?",
            params![id],
            |r| r.get(0),
        )
        .unwrap();
...
fn row_hlc(db: &DbConnection, id: &str) -> Option<String> {
    ...
    let conn = guard.as_ref().unwrap();
    conn.query_row(
        "SELECT haex_hlc_no_sync FROM devices WHERE id = ?",
        params![id],
        |r| r.get::<_, Option<String>>(0),
    )
    .unwrap()
...
fn skip_does_not_record_column_hlc_for_missing_column() {
    ...
    // column `bio`. The co-applied `name` forces the row's haex_column_hlcs_no_sync
    ...
    assert!(
        ...
        "skipped (never-applied) column must NOT have an HLC persisted in \
         haex_column_hlcs_no_sync, else post-migration recovery is gated out by the \
...
fn existing_column_newer_hlc_applies_equal_or_older_does_not() {
    // (a) newer HLC applies.
    {
        ...
        assert_eq!(
            ...
            Some(HLC_NEW),
            "row haex_hlc_no_sync must advance to the applied change's HLC"
        );
    }
