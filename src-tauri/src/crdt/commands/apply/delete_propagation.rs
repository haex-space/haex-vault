use crate::crdt::shared_space_trigger::{
    get_table_schema, is_safe_identifier, SHARED_SPACE_DELETED_ROWS_TABLE, SHARED_SPACE_SYNC_TABLE,
};
...
pub(super) fn propagate_shared_space_deleted_rows_to_target_tables(
    ...
    for (i, id) in delete_log_ids.iter().enumerate() {
        ...
        let delete_hlc = match delete_hlc_opt {
            ...
            None => {
                eprintln!(
                    "[SYNC RUST] Skipping shared-space delete-log {id}: haex_hlc_no_sync is NULL"
                );
                continue;
            }
        };
