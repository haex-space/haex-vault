use crate::crdt::shared_space_trigger::{
    get_table_schema, is_safe_identifier, DELETED_ROWS_TABLE, SHARED_SPACE_DELETED_ROWS_TABLE,
    SHARED_SPACE_SYNC_TABLE,
};
...
/// target row carries a `haex_hlc_no_sync` strictly newer than the delete-log entry,
...
pub(super) fn propagate_deleted_rows_to_target_tables(
    ...
    for id in delete_log_ids {
        let result = tx.query_row(
            &format!(
                "SELECT table_name, row_pks, haex_hlc_no_sync FROM \"{}\" WHERE id = ?1",
                DELETED_ROWS_TABLE
            ),
            params![id],
            |row| {
        ...
        // this delete-log entry, the row's haex_hlc_no_sync is strictly newer and
        // we must NOT propagate the delete.
        let select_hlc_sql = format!(
            "SELECT haex_hlc_no_sync FROM \"{}\" WHERE {}",
        ...
        if !should_propagate_delete(&delete_hlc, target_row_hlc.as_deref()) {
            eprintln!(
                "[SYNC RUST] Skipping delete-log {} for '{}': target row \
                 has newer haex_hlc_no_sync ({:?} > {}) — resurrected",
...
/// `(space_id, table_name, row_pks, haex_hlc_no_sync)` from the delete-log and:
///   1. DELETEs the target row from `table_name` with the standard
///      resurrection check (haex_hlc_no_sync > delete_hlc → skip).
...
pub(super) fn propagate_shared_space_deleted_rows_to_target_tables(
    ...
    for (i, id) in delete_log_ids.iter().enumerate() {
        let result = tx.query_row(
            &format!(
                "SELECT space_id, table_name, row_pks, haex_hlc_no_sync \
        ...
        let delete_hlc = match delete_hlc_opt {
            Some(h) => h,
            None => {
                eprintln!("[SYNC RUST] Skipping shared-space delete-log {id}: haex_hlc_no_sync is NULL");
                continue;
            }
        };
        ...
        let select_hlc_sql =
            format!("SELECT haex_hlc_no_sync FROM \"{target_table}\" WHERE {where_clause}");
        let target_row_hlc: Option<String> =
            match tx.query_row(&select_hlc_sql, param_refs.as_slice(), |row| row.get(0)) {
                Ok(hlc) => Some(hlc),
                Err(rusqlite::Error::QueryReturnedNoRows) => None,
        ...
        if !should_propagate_delete(&delete_hlc, target_row_hlc.as_deref()) {
            eprintln!(
                "[SYNC RUST] Skipping shared-space delete-log {id} for '{target_table}': \
                 target row has newer haex_hlc_no_sync ({target_row_hlc:?} > {delete_hlc}) — resurrected"
            );
            continue;
        }
...
mod tests {
    ...
    fn shared_space_delete_log_apply_removes_business_row_and_register_entry() {
        ...
        conn.execute(
            "INSERT INTO haex_shared_space_deleted_rows (id, space_id, table_name, row_pks, haex_hlc_no_sync)
    ...
    fn shared_space_delete_log_apply_preserves_resurrection_bug_free() {
        ...
        assert_eq!(
            row_count, 1,
            "resurrected row (newer haex_hlc_no_sync than delete) must be kept"
        );
    }
    ...
    fn shared_space_delete_log_apply_rejects_when_row_not_shared_in_that_space() {
        ...
        .unwrap();
        conn.execute(
            "INSERT INTO haex_calendar (id, title, haex_hlc_no_sync)
