...
impl CrdtColumns {
    ...
    fn add_to_table_definition(&self, columns: &mut Vec<ColumnDef>) {
        ...
        // `haex_column_sigs_no_trigger TEXT NOT NULL DEFAULT '{}'` so that pre-existing
...
impl CrdtTransformer {
    ...
    /// migrations to ensure all syncable tables get `haex_hlc_no_trigger` and
    /// `haex_column_hlcs_no_trigger` columns.
    ///
