...
//! - CREATE TABLE gets `haex_hlc_no_trigger` + `haex_column_hlcs_no_trigger` added
...
fn test_update_adds_hlc_assignment() {
    ...
    assert!(
        result.contains("haex_hlc_no_trigger"),
        "UPDATE must add haex_hlc_no_trigger assignment. Got: {result}"
    );
    assert!(!result.contains("haex_tombstone"), "Got: {result}");
}
...
fn test_create_table_adds_crdt_columns() {
    ...
    assert!(result.contains("haex_hlc_no_trigger"), "Got: {result}");
    assert!(result.contains("haex_column_hlcs_no_trigger"), "Got: {result}");
    assert!(
...
fn transform_ddl_injects_haex_column_sigs_on_space_scoped_tables() {
    ...
    // incoming column change. Parallel to `haex_column_hlcs_no_trigger`, but with an
    ...
    assert!(
        out.contains("haex_column_sigs_no_trigger"),
        "space-scoped CREATE TABLE must gain `haex_column_sigs_no_trigger`. Got: {out}"
    );
    assert!(
        out.contains("DEFAULT '{}'"),
        "`haex_column_sigs_no_trigger` must default to an empty JSON object. Got: {out}"
    );
    // Sig column must not accidentally displace the HLC columns.
    assert!(out.contains("haex_hlc_no_trigger"), "Got: {out}");
    assert!(out.contains("haex_column_hlcs_no_trigger"), "Got: {out}");
}
...
fn transform_ddl_skips_haex_column_sigs_for_no_sync_tables() {
    ...
    assert!(
        !out.contains("haex_column_sigs_no_trigger"),
        "_no_sync tables must not gain `haex_column_sigs_no_trigger`. Got: {out}"
    );
}
...
fn haex_logs_no_sync_is_not_a_crdt_sync_table() {
    ...
    // selects tables by presence of the `haex_hlc_no_trigger` column, so keeping that
    ...
    assert!(
        !result.contains("haex_hlc_no_trigger"),
        "haex_logs_no_sync must not gain `haex_hlc_no_trigger` — that column is what discover_crdt_tables keys on. Got: {result}"
    );
    assert!(
        !result.contains("haex_column_hlcs_no_trigger"),
        "haex_logs_no_sync must not gain `haex_column_hlcs_no_trigger`. Got: {result}"
    );
}
...
fn test_insert_into_sync_table_gets_hlc_column() {
    ...
    // InsertTransformer adds haex_hlc_no_trigger as a literal column/value
    assert!(result.contains("haex_hlc_no_trigger"), "Got: {result}");
}
