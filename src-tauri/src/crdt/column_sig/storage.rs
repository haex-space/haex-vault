//! Storage helper for the `haex_column_sigs_no_sync` JSON meta column.
//!
//! `haex_column_sigs_no_sync` on each space-scoped table stores per-column,
...
/// Merges a signature into the `haex_column_sigs_no_sync` JSON column of the row
...
pub fn upsert_column_sigs(
    ...
    let select_sql = format!(
        "SELECT haex_column_sigs_no_sync FROM \"{}\" WHERE {}",
    ...
    let mut root: Map<String, Value> = match serde_json::from_str::<Value>(&current_raw) {
        ...
        _ => {
            ...
            warn!(
                ...
                table = table_name,
                column = column_name,
                "haex_column_sigs_no_sync root is not a JSON object — resetting to empty map"
            );
            Map::new()
        }
    };
    ...
    let column_map = match column_entry {
        ...
        other => {
            ...
            warn!(
                ...
                table = table_name,
                column = column_name,
                "haex_column_sigs_no_sync[column] is not a JSON object — refusing to clobber; resetting to empty map"
            );
    ...
    let update_sql = format!(
        "UPDATE \"{}\" SET haex_column_sigs_no_sync = ? WHERE {}",
        table_name, where_clause
    );
