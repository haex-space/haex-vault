//! Tauri commands for extension shared space assignments.
//!
//! These commands allow extensions to:
//! - Assign/unassign their table rows to shared spaces for selective sync
//!
//! Space listing, creation, and backend queries are handled by the frontend
//! stores (useSpacesStore, useSyncBackendsStore) which use Supabase JWT auth.
