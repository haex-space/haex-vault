...
impl PermissionManager {
    ...
    pub async fn check_web_permission(
        ...
        let sql = format!(
            "SELECT id, principal_id, resource_type, action, target, constraints, status, haex_hlc_no_sync FROM {TABLE_PRINCIPAL_PERMISSIONS} WHERE principal_id = ? AND resource_type = 'web'"
        );
