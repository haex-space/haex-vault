...
fn table_has_no_crdt_columns() {
    ...
    // gain `haex_hlc_no_sync` / `haex_column_hlcs_no_sync`, otherwise discover_crdt_tables
    ...
    let cols: Vec<String> = stmt
        ...
        .collect();
    assert!(
        !cols.iter().any(|c| c == "haex_hlc_no_sync"),
        "haex_hlc_no_sync leaked into haex_logs_no_sync. Cols: {cols:?}"
    );
    assert!(
        !cols.iter().any(|c| c == "haex_column_hlcs_no_sync"),
        "haex_column_hlcs_no_sync leaked into haex_logs_no_sync. Cols: {cols:?}"
    );
}
