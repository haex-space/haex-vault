...
/// `key_cache` signiert und über `haex_column_sigs_no_sync` persistiert.
...
pub fn execute_with_crdt(
    ...
    // `haex_hlc_no_sync` silently, and — critically — a caller-supplied
    // `haex_column_hlcs_no_sync = '{"col":"9999-…"}'` would feed a forged HLC into
...
/// Post-write signing pass: for every row that carries `haex_hlc_no_sync == tx_hlc`
...
fn sign_written_rows(
    ...
    // haex_hlc_no_sync on every row it just touched, so we use it as the WHERE key.
...
/// (identified by matching `haex_hlc_no_sync` against the tx HLC), do:
///
...
///    stored `haex_column_hlcs_no_sync[col]` — we sign columns AS THEY EXIST, with
///    their historical timestamps, not the tx HLC of the register INSERT.
/// 4. Upsert each `(column, space_id_new)` sig into the row's `haex_column_sigs_no_sync`.
///
...
fn sign_share_insert_targets(
    ...
    // Guard: F2 needs the register carrying `haex_hlc_no_sync` to identify the
    ...
    if !has_hlc {
        tracing::warn!(
            ...
            "F2 sig path skipped: share register is missing CRDT meta \
             (`haex_hlc_no_sync`) — schema drift or unmigrated fixture. \
    ...
    for share in share_rows {
        ...
        // SELECT all non-PK, non-CRDT-meta columns plus haex_column_hlcs_no_sync for HLC lookup.
...
fn sign_registry_row_self(
    ...
    // register table that (unusually) lacks `haex_column_sigs_no_sync` — F1 already
