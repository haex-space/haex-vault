...
// under `haex_column_sigs_no_sync` for every space the row is shared into.
...
fn setup_fixture() -> Fixture {
    ...
    conn.execute(
        "UPDATE haex_shared_space_sync SET haex_column_sigs_no_sync = ?1 WHERE id = 'share-A'",
        [self_authored_register_routing_sigs("space_A", &did_a)],
    )
    .unwrap();
    conn.execute(
        "UPDATE haex_shared_space_sync SET haex_column_sigs_no_sync = ?1 WHERE id = 'share-B'",
...
fn read_column_sigs_json(
    ...
    match v {
        ...
        other => panic!("haex_column_sigs_no_sync is not a JSON object: {other:?}"),
    }
}
...
/// `haex_column_sigs_no_sync["mixedcase"]` with a preimage built over `b"mixedcase"`,
...
fn sign_written_rows_covers_mixedcase_ddl_columns() {
    ...
    let row_hlc: String = {
        ...
        let conn = guard.as_ref().unwrap();
        conn.query_row(
            "SELECT haex_hlc_no_sync FROM ext_calendar WHERE id = 'R'",
            [],
            |r| r.get(0),
        )
        .unwrap()
...
/// `execute_with_crdt` UPDATE — that path populates `haex_hlc_no_sync` and
/// `haex_column_hlcs_no_sync` the same way as a live vault would. F2 then reads
...
fn insert_into_share_register_signs_all_columns_of_referenced_row() {
    ...
    let title_hlc: String = {
        ...
        let conn = guard.as_ref().unwrap();
        conn.query_row(
            "SELECT json_extract(haex_column_hlcs_no_sync, '$.title') \
    ...
    assert!(
        !title_hlc.is_empty(),
        "seed step must populate haex_column_hlcs_no_sync.title"
    );
...
/// *same* `haex_column_sigs_no_sync` JSON key for a mixed-case DDL column, since both
...
fn execute_with_crdt_insert_without_column_list_signs_all_columns() {
    ...
    // `haex_hlc_no_sync` to the column list, so `VALUES (?1, ?2)` here binds
    // `id` and `title`.
    let result = core::execute_with_crdt(
...
fn execute_with_crdt_signs_composite_pk_row() {
    ...
    // column `label`.
    {
        ...
        conn.execute(
            "INSERT INTO ext_composite (space_ref, item_id, label, haex_column_hlcs_no_sync, haex_column_sigs_no_sync) \
             VALUES (?1, ?2, ?3, '{}', '{}')",
    ...
    let raw: String = {
        ...
        let conn = guard.as_ref().unwrap();
        conn.query_row(
            "SELECT haex_column_sigs_no_sync FROM ext_composite \
...
/// F#2: a caller-supplied `haex_column_hlcs_no_sync` assignment must be rejected.
...
fn execute_with_crdt_rejects_user_supplied_haex_column_hlcs() {
    ...
    let result = core::execute_with_crdt(
        "UPDATE ext_calendar SET haex_column_hlcs_no_sync = ?1 WHERE id = ?2".to_string(),
        vec![
    ...
    match result {
        Err(crate::database::error::DatabaseError::CrdtMetaColumnWriteForbidden { column }) => {
            assert_eq!(column, "haex_column_hlcs_no_sync");
        }
        other => panic!("expected CrdtMetaColumnWriteForbidden, got: {:?}", other),
    }
}

/// F#2: caller-supplied `haex_hlc_no_sync` on INSERT must be rejected — even
...
fn execute_with_crdt_rejects_user_supplied_haex_hlc_on_insert() {
    ...
    let result = core::execute_with_crdt(
        "INSERT INTO ext_calendar (id, title, haex_hlc_no_sync) VALUES (?1, ?2, ?3)".to_string(),
        vec![
    ...
    match result {
        Err(crate::database::error::DatabaseError::CrdtMetaColumnWriteForbidden { column }) => {
            assert_eq!(column, "haex_hlc_no_sync");
        }
        other => panic!("expected CrdtMetaColumnWriteForbidden, got: {:?}", other),
    }
}
...
/// naming `HAEX_HLC_NO_SYNC` (any case) sailed through unnoticed. Fixed by
...
fn execute_with_crdt_rejects_user_supplied_uppercase_haex_hlc_on_insert() {
    ...
    match result {
        Err(crate::database::error::DatabaseError::CrdtMetaColumnWriteForbidden { column }) => {
            assert_eq!(
                column, "haex_hlc_no_sync",
                "column name is case-folded to lowercase"
            );
        }
        other => panic!(
            "expected CrdtMetaColumnWriteForbidden for uppercase HAEX_HLC_NO_SYNC, got: {:?}",
            other
        ),
    }
}

/// F#2: `haex_column_sigs_no_sync` is also managed by the sig layer — a caller
...
fn execute_with_crdt_rejects_user_supplied_haex_column_sigs() {
    ...
    let result = core::execute_with_crdt(
        "UPDATE ext_calendar SET haex_column_sigs_no_sync = ?1 WHERE id = ?2".to_string(),
        vec![
    ...
    match result {
        Err(crate::database::error::DatabaseError::CrdtMetaColumnWriteForbidden { column }) => {
            assert_eq!(column, "haex_column_sigs_no_sync");
        }
        other => panic!("expected CrdtMetaColumnWriteForbidden, got: {:?}", other),
    }
}
...
fn setup_fixture_s3_backends() -> FixtureS3Backends {
    ...
    conn.execute(
        "INSERT INTO haex_s3_backends (id, endpoint, haex_column_hlcs_no_sync, haex_column_sigs_no_sync) \
         VALUES (?1, ?2, '{}', '{}')",
