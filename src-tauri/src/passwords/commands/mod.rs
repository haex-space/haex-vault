    ...
    let summaries: Vec<PasswordItemSummary> = rows
        ...
        .map(|row| {
            ...
            PasswordItemSummary {
                ...
                created_at_no_sync: non_empty(get_string(row, 6)),
                updated_at_no_sync: non_empty(get_string(row, 7)),
                tags,
            }
        })
        .collect();
...
pub async fn extension_password_read(
    ...
    Ok(PasswordItemFull {
        ...
        created_at_no_sync: non_empty(get_string(row, 14)),
        updated_at_no_sync: non_empty(get_string(row, 15)),
        tags,
        key_values,
    })
}
