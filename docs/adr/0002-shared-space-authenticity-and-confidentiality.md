...
- **Vault-private Tabellen:** alle `haex_*`-System-Tabellen. Kreuzen **nie** einen
  Space-Stream.
...
2. Space-Infrastruktur (die 5) → sync **1:1** via `space_id`-Spalte.
3. Erweiterungs-Daten + Dateien → **M:N** via Share-Register (+ File-Sync).
...
- **Vertraulichkeit:** Nur aktuelle Space-Member können den Content entschlüsseln;
  Leader/Relay sehen nur Ciphertext.
...
- **Kanonische Serialisierung (§6.2 gelöst):** KEINE JSON-Kanonisierung. Domain-separierte,
  ...
  dann ed25519. **`value_bytes` = exakt die gespeicherte/übertragene Byte-Form** (nie
  re-serialisiertes JSON). `domain_tag` = fester Präfix `"haex/space-col-sig/v1"` gegen
  Cross-Protocol-Reuse. ⚠️ Impl-Risiko: Value darf durch die Pipeline nie re-serialisiert
  werden (`1.0`→`1`); signierte Bytes verbatim mitführen.
...
- **Verifier:** jeder Apply-Pfad (`apply_remote_changes_to_db` als Choke-Point)
  verifiziert **vor** dem Merge gegen den aus `author_did` abgeleiteten Public Key.
  Ungültig/fehlend → Change verworfen.
...
1. **Space-Identität self-certifying machen (Phase 0, siehe §6.1):**
   `space_id = base58(hash(root_did || nonce))`. Root-UCAN nur gültig, wenn seine
   Issuer-DID zu `space_id` hasht. Damit ist die Wurzel unfälschbar aus der id ableitbar.
2. **`prf`-Kette bei der Verifikation laufen:** Member-Token ← Admin-Delegation ←
   ...
   Space-Root. Ersetzt das Vertrauen in die `capability`-DB-Spalte.
...
Klartext, übertragen/gespeichert der Ciphertext.**
...
Infra-Tabellen via `space_id`; **alle anderen `haex_*` kategorisch verboten auf beiden
Pfaden.** `is_space_scoped_table()` wird zu dieser zusammengesetzten Prüfung, die
System-Tabellen weiterhin ausschließt.
...
Fehlt heute (Zufalls-UUID). Fix: self-certifying `space_id = base58(hash(root_did ||
nonce))` bei Erstellung (`createLocalSpace`/`createOnlineSpace`), Verifier prüft
id↔root-DID-Bindung. Keine Migration (keine Prod-Nutzer).
...
- **Phase 0 — Self-certifying space_id.** `space_id = base58(hash(root_did || nonce))`,
  Verifier prüft Bindung. *Fundament für Option 2; ohne dies kein vertrauenswürdiger Anker.*
- **Phase 1 — Per-(Spalte, space_id) Autor-Signatur (4b)** auf allen space-scoped Tabellen
  (die 5 Infra-Tabellen zuerst): `haex_column_sigs`, Signieren in `execute_with_crdt`,
  Verifizieren im Apply, `authored_by_did` löschen + Stubs nach Rust.
- **Phase 2 — UCAN-Delegations-Ketten-Verifikation (4c):** `prf` laufen, Root-Anker.
...
- Genaues Encoding der self-certifying `space_id` (Hash-Funktion, nonce-Länge, base58 vs.
  base32) + wo der Verifier die Bindung prüft (Phase 0).
